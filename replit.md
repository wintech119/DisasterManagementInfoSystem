...
- **Item Movement Detail Dashboard**: Sub-dashboard under Aid Movement for drilling down into a specific item's movement across warehouses. Features item selection dropdown (required), warehouse filter, movement type filter, date range filters, 3 summary KPI cards (Total Received, Total Issued, In Store for selected item), and per-warehouse breakdown table showing received/issued/in-store quantities. Route: `/dashboard/aid-movement/item-detail`. Accessible to DG, Deputy DG, Director PEOD, and Logistics Manager roles.
