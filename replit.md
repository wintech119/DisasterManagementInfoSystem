...
- **COMPLETE Timezone Fix - System-Wide Correction**: Fixed critical timezone bug where `datetime.now()` was storing UTC timestamps instead of Jamaica Standard Time across the entire application. **All fixes completed**: (1) `app/core/audit.py` - all audit field timestamps (create_dtime, update_dtime, verify_dtime), (2) `app/db/models.py` - DistributionPackage model defaults, (3) `app/features/packaging.py` - 29 timestamp operations, (4) `app/features/donations.py` - 4 verification timestamps, (5) `app/features/donation_intake.py` - intake timestamps, (6) `app/features/operations_dashboard.py` - dashboard date calculations, (7) `app/features/reports.py` - export filename timestamps. **ROOT CAUSE**: System timezone environment variable is NOT set, so Python's `datetime.now()` returns UTC time. **SOLUTION**: All 37 `datetime.now()` occurrences replaced with `jamaica_now()` from `app/utils/timezone`. **IMPACT**: All new database records now store correct Jamaica Standard Time timestamps; existing records may have UTC timestamps (non-breaking, cosmetic only). **RECOMMENDED NEXT STEPS**: (1) Add CI lint check to prevent new `datetime.now()` regressions, (2) Optional: run data audit script to quantify existing UTC-stamped rows. Architect-reviewed and approved.
