...
- **Submit/Delete Button Fix**: Fixed "Submit to ODPEM" and "Delete Draft" buttons not working due to blocked browser confirm() dialogs. Root cause: Browser's native confirm() dialogs being blocked or auto-dismissed. Solution: Replaced all native confirm() dialogs with Bootstrap modals throughout the application. Implemented for requests/edit_items.html with clear, visible confirmation modals (green header for submit, red for delete), detailed explanations of consequences, and proper form submission on confirmation. JavaScript prevents default form submission, shows modal, then submits form when user confirms. Includes console logging for debugging. Other pages (agency_requests, view pages, etc.) pending similar fixes.
