...
(
	...
	--Optional event for which the transfer is being done
	event_id integer
		constraint fk_transfer_event references event,
...
(
	...
	status_code char(1) not null
		--A=Active, I=Inactive
		constraint c_agency_5 check (status_code in ('A','I')),
...
(
	...
	eligible_event_id integer
		constraint fk_reliefrqst references event,
...
(
	...
	status_code char(1) not null
		...
		--P=Processing, C=Completed, V=Verified, D=Dispatched, R=Received
		constraint c_reliefpkg_3 check (status_code in ('P','C','V','D','R')),
...
--
--Intake of items into inventories of agencies/shelters
--During intake, items are inspected and classified as usable/defective
--and assigned to required locations.
--
create table pkgintake
(
	reliefpkg_id integer not null
		constraint fk_pkgintake_reliefpkg references reliefpkg,

	inventory_id integer not null
		constraint fk_pkgintake_inventory references inventory,

	intake_date date not null
		constraint c_pkgintake_1 check (intake_date <= CURRENT_DATE),
	comments_text varchar(255),

	status_code char(1) not null
		--I=Incomplete, C=Completed, V=Verified
		constraint c_pkgintake_2 check (status_code in ('I','C','V')),
...
--NOTE: Intake of items must not reference inventory as items being taken to the
--inventory may not yet exist in it.	version_nbr integer not null
	constraint pk_pkgintake primary key(reliefpkg_id,inventory_id)
);
...
(
	...
	--Quantity/amount of usable/good item in inventory
	usable_qty decimal(12,2) not null
		constraint c_pkgintake_item_1 check (usable_qty >= 0.00),
	location1_id integer --not null
		constraint fk_pkgintake_item_location1 references location,
	
	--Quantity/amount of defective item in inventory
	defective_qty decimal(12,2) not null
		constraint c_pkgintake_item_2 check (defective_qty >= 0.00),
	location2_id integer --not null
		constraint fk_pkgintake_item_location2 references location,
	
	--Quantity/amount of expired item in inventory
	expired_qty decimal(12,2) not null
		constraint c_pkgintake_item_3 check (defective_qty >= 0.00),
	location3_id integer --not null
		constraint fk_pkgintake_item_location3 references location,

	--Units in which quantity of item is measured
	uom_code varchar(25) not null
		constraint fk_pkgintake_item_unitofmeasure references unitofmeasure,

	status_code char(1) not null
		--P=Pending verification, V=Verified
		constraint c_pkgintake_item_4 check (status_code in ('P','V')),
	comments_text varchar(255),
	...
	constraint fk_pkgintake_item_intake foreign key (reliefpkg_id,inventory_id) 
		references pkgintake(reliefpkg_id,inventory_id),
	constraint pk_pkgintake_item primary key(reliefpkg_id,inventory_id,item_id)
);
create index dk_pkgintake_item_1 on pkgintake_item(inventory_id, item_id);
create index dk_pkgintake_item_2 on pkgintake_item(item_id);
