...
-- Date: 2025-11-26
-- Purpose: Add a new table to cache exchange rates from Frankfurter.app (ECB-backed)
--          for converting foreign currencies to JMD for display purposes.
-- 
...
(
    ...
    source          VARCHAR(50) NOT NULL DEFAULT 'FRANKFURTER_ECB',
...
COMMENT ON TABLE currency_rate IS 'Cached exchange rates to JMD from Frankfurter.app (ECB-backed). Used for display-only currency conversion.';
...
COMMENT ON COLUMN currency_rate.source IS 'Rate source identifier, default is FRANKFURTER_ECB';
